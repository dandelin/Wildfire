...
def test_nms():
    ...
    print(f'numba_nms : min {min(numba_time) * 1e+6 / 10**3:.4f}us, '
          f'max {max(numba_time) * 1e+6 / 10**3:.4f}us, '
          f'sum {sum(numba_time) * 1e+6 / 10**3:.4f}us '
          f'cuda_nms : min {min(native_time) * 1e+6 / 10**3:.4f}us, '
          f'max {max(native_time) * 1e+6 / 10**3:.4f}us, '
          f'sum {sum(native_time) * 1e+6 / 10**3:.4f}us')
